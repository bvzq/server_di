...
def cv(X,y,params,cv_splits, loss = tf.keras.losses.BinaryCrossentropy(),dims=None,isNN=True):
    #***CNN****
    if isNN:
        print("isNN")
...
def cv_recall(X,y,params,cv_splits,dims=None,isNN=True):
    #***CNN****
    if isNN:
        print("isNN")
        kf = KFold(n_splits=cv_splits,shuffle=True,random_state=0)
...
print("varroa com recall")
n_splits = 4
histories = cv_recall(X_train_varroa,y_train_varroa,params,n_splits,dims)
...
print("multi job")
n_splits = 4
histories = cv(X_train_multi,y_train_multi,params, tf.keras.losses.CategoricalCrossentropy(),n_splits,dims)
